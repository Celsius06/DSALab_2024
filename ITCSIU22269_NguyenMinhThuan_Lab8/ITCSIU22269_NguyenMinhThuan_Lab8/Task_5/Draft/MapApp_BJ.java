...
public class MapApp_BJ {
    ...
    static class Graph {
        ...
        public List<Character> shortestPath(int start, int target) {
            ...
            for (int count = 0; count < nVerts - 1; count++) {
                ...
                for (int i = 0; i < nVerts; i++) {
                    if (!visited[i] && adjMat[closestVertex][i] != 0 &&
                            distance[closestVertex] != Integer.MAX_VALUE &&
                            distance[closestVertex] + adjMat[closestVertex][i] < distance[i]) {
                        distance[i] = distance[closestVertex] + adjMat[closestVertex][i];
    ...
    public static void main(String[] args) {
        ...
        List<Character> shortestPath = graph.shortestPath(1, 9); // 1: B, 9: J

        // Print the shortest path
        System.out.print("[Dijkstra] Shortest path from B to J: ");
